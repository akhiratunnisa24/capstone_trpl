                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading  col-sm-15 clearfix">
                            <div class="pull-right">
                                <a href="" class="btn btn-dark btn-sm fa fa-plus" data-toggle="modal"
                                    data-target="#addslip"> Tambah Slip Baru</a>
                            </div>
                        </div>
                        <div class="panel-body m-b-5">
                            <div class="row">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <table id="datatable-responsive49"
                                        ...
                                        <tbody>
                                            @foreach ($slipgaji as $data)
                                                <tr>
                                                    ...
                                                    <td>{{ \Carbon\Carbon::parse($data->tglawal)->format('d/m/Y') }} -
                                                        {{ \Carbon\Carbon::parse($data->tglakhir)->format('d/m/Y') }}</td>
                                                    <td>{{ $data->karyawans->nama }}</td>
                                                    <td>{{ $data->karyawans->nama_jabatan }}</td>
                                                    {{-- <td>{{ number_format($data->gaji_pokok, 0, ',', '.') }}</td> --}}
                                                    <td>{{ \Carbon\Carbon::parse($data->karyawans->tglmasuk)->format('d/m/Y') }}
                                                    </td>
                                                    <td>
                                                        <div class="d-grid gap-2" role="group" aria-label="Basic example">
                                                            <form method="POST" action="/slipgaji{{ $data->id }}"
                                                                ...
                                                                <input type="hidden" name="id_karyawan"
                                                                    value="{{ $data->karyawans->id }}">
                                                                <input type="hidden" name="id"
                                                                    value="{{ $data->id }}">
                                                                <button type="submit" class="btn btn-success btn-sm"
                                                                    title="Lihat Slip Gaji"><i
                                                                        class="fa fa-eye"></i></button>
                                                            </form>
                                                            <button class="btn btn-danger btn-sm col-md-4"
            ...
            swal.fire({
                ...
                if (result.isConfirmed) {
                    swal.fire({
                        ...
                        text: "Data Slip Gaji sedang diperiksa",
                        icon: "info",
                        confirmButtonColor: '#3085d6',
                    })
                    location.href = '<?= '/slip-gaji-delete/' ?>' + id;
                }
            })
        }
    </script>
