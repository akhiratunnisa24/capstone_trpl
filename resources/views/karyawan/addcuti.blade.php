...
<link href="assets/plugins/bootstrap-datepicker/css/bootstrap-datepicker.min.css" rel="stylesheet">
    ...
    <div class="modal modal fade" id="mModal" tabindex="-1" role="dialog" aria-labelledby="Modal" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('store.sisa')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
    ...
    <script  type="text/javascript">
        ...
        $('#id_jeniscuti').on('change',function(e){
            ...
            $.ajax({
                type:"POST",
                url: '{{route('get.Sisacuti')}}',
                data: {'id_jeniscuti':id_jeniscuti},
                success:function(data)
                {
                    $('#id_alokasi').val(data.id_alokasi);
                    $('#id_jeniscutis').val(data.jenis_cuti)
                    $('#jeniscuti').val(data.jeniscutis);
                    $('#id_settingalokasi').val(data.id_settingalokasi);
                    $('#durasi').val(data.sisa_cuti);
    ...
    <script type="text/javascript">
        function jumlahcuti()
        {
            var start= $('#datepicker-autocloseq').val();
            var end  = $('#datepicker-autocloser').val();
