        ...
        if(isset($row['id_karyawan']) && isset($row['id_jeniscuti']))
        {
            ...
            if(!Alokasicuti::where('id_karyawan',$row['id_karyawan'])->where('id_jeniscuti',$row['id_jeniscuti'])->exists())
            {
                $data = [
                     ...
                     'tgl_masuk'        => \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row['tanggal_masuk'])->format("Y-m-d") ?? null,
                     'tgl_sekarang'     => \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row['tanggal_sekarang'])->format("Y-m-d") ?? null,
                 ...
                 // dd($data);
                  Log::info($row['id_karyawan'],['id_jeniscuti'],$row['tanggal_masuk']);
